...
let fold_right (b2 : 'b parsing) (r1 : 'a parsing) (f : 'b -> 'a -> 'a) : 'a parsing =
  fun prio p_state c_state ->
    let rec aux c_state =
      ...
      | Some (v, c_state') -> begin match aux c_state' with
        | Some (v', c_state) -> Some (f v v', c_state)
        | None -> r1 prio p_state c_state
      end
    in aux c_state
  ...
  let rec aux2 p =
    ...
    then None
      ...
      | RightAssoc possibilities ->
        begin match fold_rightL self possibilities (p - 1) state s with
          | None -> aux2 (p - 1)
          | out -> out
        end
    in aux2 p
